...
class MqttBroker {
    ...
    constructor({ eventsTopic, brokerUrl }) {
        ...
        this.orderedIncomingEvents$ = this.incomingEvents$
            .filter(msg => msg)
    ...
    publish$(data) {
        ...
        return Rx.Observable.defer(() => this.mqttClient.publish(`${this.topicName}`, dataBuffer, { qos: 1 }))
            .mapTo(uuid);
    }
    ...
    getEventListener$(aggregateType, ignoreSelfEvents = true) {
        return this.orderedIncomingEvents$
            .filter(msg => msg)
