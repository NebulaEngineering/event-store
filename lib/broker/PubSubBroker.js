        ...
        this.messageListenerSubscription =
            this.aggregateEventsMapConfiguredSubject$.pipe(
                ...
                filter(() => {
                    ...
                    const canAcceptMoreMessages = this.retainedMessagesCount < EVENT_STORE_BROKER_MAX_MESSAGES;
                    if (this.waitingForPullingMessageCompletion || !canAcceptMoreMessages) {
                        return false;
                    }
                ...
                mergeMap(([response]) => {
                    ...
                    if ((pullingTime) > EVENT_STORE_BROKER_STATS_SLOW_PULL) {
                        ...
                        ConsoleLogger.w(`EventStore.PubSubBroker.startMessageListener: subscriptionClient.pull(${messages.length}) is too slow: ${pullingTime}ms, dataLen=${dataLen}`);
                    }
                    if (EVENT_STORE_BROKER_STATS_PULL_ACK_LEN > 0) {
                        if (dataLen <= 0) {
                            for (let receivedMessage of response.receivedMessages) {
                ...
                (rawData) => {
                    ...
                    if (this.retainedMessagesMetadata[rawData.message.messageId]) {
                        ...
                        ConsoleLogger.w(`EventStore.PubSubBroker.startMessageListener: event with id=${rawData.message.messageId}, state=${msgMetadata.state} ackCount=${msgMetadata.ackCnt} resent by pubsub. ackId had been updated.`);
                        const isInemdiateAckStates = [
                            MSG_STATES.ACK_ERROR,
                            MSG_STATES.IGNORED,
                            MSG_STATES.IGNORED_BPAR,
                            MSG_STATES.IGNORED_PPF,
                            MSG_STATES.IGNORED_APAR,
                            MSG_STATES.IGNORED_SELF,
                            MSG_STATES.IGNORED_SENT,
                            MSG_STATES.AUTO_ACK,
                            MSG_STATES.PROCESS_DONE
                        ];
                        if (isInemdiateAckStates.includes(msgMetadata.state)) {
            ...
            mergeMap((buffer) => {
                ...
                return of(messagesToAckIds).pipe(
                    ...
                    tap((result) => {
                        ...
                        this.retainedMessagesCount -= messagesToAckIds.length - Object.keys(faultyIckIdMap).length;
                        for (let msg of buffer) {
                    ...
                    retryWhen((errors) =>
                        errors.pipe(
                            tap((err) => {
                                ...
                                if (faultyAckId != null) {
                                ...
                                } else if (err.message.includes('DEADLINE_EXCEEDED') || err.message.includes('closed')) {
                                    //sometime pubsub respond with the error DEADLINE_EXCEEDED, when this happens we have to restart the client
                                    // in order to do so we send a signal to ditch the current listener an create a new one
                                    ...
                                    ConsoleLogger.e(`EventStore.PubSubBroker.startMessageAcknowledged: unexpected error while send acknowledges messages.`, err);
                                }
                                retryAttempt += 1;
