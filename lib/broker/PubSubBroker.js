...
class PubSubBroker {
    ...
    getEventListener$(aggregateType, ignoreSelfEvents = true) {
        const allowAll = Array.isArray(aggregateType) ? aggregateType.includes('$ALL') : aggregateType === '$ALL';
        ...
        return this.orderedIncomingEvents$.pipe(
            ...
            filter(evt => allowAll || evt.at === aggregateType || aggregateType == "*")
        );
                ...
                (pubSubSubscription) => {
                    this.onMessage = message => {
                        ...
                        if (!eventTypeConfig) {
                            ...
                            eventTypeConfig = atMap[msgEvt.data.et] || atMap.$ALL;
                        }
                        if (!eventTypeConfig) {
