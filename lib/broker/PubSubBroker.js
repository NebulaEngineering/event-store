...
const EVENT_STORE_BROKER_MAX_PUBLISHER_BATCH_MAX_MILLIS = parseInt(process.env.EVENT_STORE_BROKER_MAX_PUBLISHER_BATCH_MAX_MILLIS || "10000");
const EVENT_STORE_BROKER_MAX_EXTENSION = parseInt(process.env.EVENT_STORE_BROKER_MAX_EXTENSION || "1500");
const EVENT_STORE_BROKER_MAX_RETRY_ATTEMPS = parseInt(process.env.EVENT_STORE_BROKER_MAX_RETRY_ATTEMPS || "3", 10);
...
const EVENT_STORE_PULL_MESSAGE_AMOUNT = parseInt(process.env.EVENT_STORE_PULL_MESSAGE_INTERVAL_MILLIS || "500", 10);
const EVENT_STORE_SUBSCRIPTION_ACKDEADLINE = parseInt(process.env.EVENT_STORE_SUBSCRIPTION_ACKDEADLINE || "500", 10);
class PubSubBroker {
    ...
    start$() {
        return new Observable(observer => {
            ...
            this.intervalID = setInterval(
                (function (self) {
                    return function () {
                        ConsoleLogger.w(`EventStore.PubSubBroker.start$: Unacked cronjob executed: ${self.unackedMessages ? Object.keys(self.unackedMessages).length : 'null'}`);
        ...
        return defer(() => this.subscriptionClient.getSubscription({ subscription: [this.formattedSubscription] })).pipe(
            catchError((err) => {
                const request = {
                    ...
                    ackDeadlineSeconds: EVENT_STORE_SUBSCRIPTION_ACKDEADLINE
                };
            ...
            this.aggregateEventsMapConfiguredSubject$.pipe(
                ...
                filter(() => {
                    if (this.msgWithoutAckAmount > EVENT_STORE_BROKER_MAX_MESSAGES && !this.shouldShowStopPullingMessage) {
                        ...
                        ConsoleLogger.e(`EventStore.PubSubBroker.startMessageListener: Stop pulling messages because of messages without ack limit exceeded: ${this.msgWithoutAckAmount}, of ${EVENT_STORE_BROKER_MAX_MESSAGES}`);
                    }
                    if (this.msgWithoutAckAmount < EVENT_STORE_BROKER_MAX_MESSAGES && this.shouldShowStopPullingMessage) {
                        this.shouldShowStopPullingMessage = false;
                        ConsoleLogger.i(`EventStore.PubSubBroker.startMessageListener: Start pulling messages because of messages without ack limit flatted: ${this.msgWithoutAckAmount}, of ${EVENT_STORE_BROKER_MAX_MESSAGES}`);
                    }
                ...
                (err) => {
                    ConsoleLogger.e(`EventStore.PubSubBroker.startMessageListener: Failed to obtain EventStore subscription`, err);
                    process.exit(1);
                },
                () => {
                    this.isStopped = true;
                    ConsoleLogger.e('EventStore.PubSubBroker.startMessageListener: listener has completed!');
                }
            );
    }
                ...
                return of(messagesToAckIds).pipe(
                    ...
                    retryWhen((errors) =>
                        errors.pipe(
                            tap((err) => {
                                retryAttempt += 1;
                                if (retryAttempt === EVENT_STORE_BROKER_MAX_RETRY_ATTEMPS) {
