...
const { filter, map, tap, mergeMap, last, mapTo, bufferTime, retryWhen, delayWhen, concatMap, takeUntil, catchError, first, timeout } = require('rxjs/operators');
                    ...
                    retryWhen((errors) =>
                        errors.pipe(
                            tap((err) => {
                                ...
                                ConsoleLogger.w(`EventStore.PubSubBroker.startMessageAcknowledged: error while pulling messages form pubsub. message=${err.message}, faultyAckId=${faultyAckId}, will remove faulty ackId and proceed`, err);
                                if (faultyAckId != null) {
