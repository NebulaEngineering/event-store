...
const EVENT_STORE_STATS_PERIOD = parseInt(process.env.EVENT_STORE_STATS_PERIOD || "300000", 10);
const EVENT_STORE_STATS_OLD_MSG_THRESHOLD = parseInt(process.env.EVENT_STORE_STATS_OLD_MSG_THRESHOLD || "120000", 10);
const EVENT_STORE_STATS_OLD_MSG_STATE_FILTER = process.env.EVENT_STORE_STATS_OLD_MSG_STATE_FILTER || Object.values(MSG_STATES).join(',');
const EVENT_STORE_STATS_PULL_ACK_LEN = parseInt(process.env.EVENT_STORE_STATS_PULL_ACK_LEN || "0", 10);
const EVENT_STORE_STATS_SLOW_ACK = parseInt(process.env.EVENT_STORE_STATS_PULL_ACK_LEN || "2000", 10);
const EVENT_STORE_STATS_SLOW_PULL = parseInt(process.env.EVENT_STORE_STATS_PULL_ACK_LEN || "2000", 10);
...
class PubSubBroker {
    ...
    start$() {
        return new Observable(observer => {
            ...
            if (EVENT_STORE_STATS_PERIOD > 0)
                this.intervalID = setInterval(
                    (function (self) {
                        return function () {
                            if (EVENT_STORE_STATS_PULL_ACK_LEN > 0) {
                            ...
                            const msgStatsToShow = Object.values(self.retainedMessagesMetadata)
                                .filter(msg => ((Date.now() - msg.srvTs) > EVENT_STORE_STATS_OLD_MSG_THRESHOLD) && EVENT_STORE_STATS_OLD_MSG_STATE_FILTER.includes(msg.state))
                                .map(msg => {
        ...
        this.messageListenerSubscription =
            this.aggregateEventsMapConfiguredSubject$.pipe(
                ...
                mergeMap(([response]) => {
                    ...
                    if((pullingTime) > EVENT_STORE_STATS_SLOW_PULL){
                    ...
                    if (EVENT_STORE_STATS_PULL_ACK_LEN > 0) {
                        ...
                        this.pullStats = this.pullStats.slice(-1 * EVENT_STORE_STATS_PULL_ACK_LEN)
                    }
            ...
            mergeMap((buffer) => {
                ...
                return of(messagesToAckIds).pipe(
                    ...
                    tap((result) => {
                        ...
                        if((ackTimeDiff) > EVENT_STORE_STATS_SLOW_ACK){
                            ConsoleLogger.w(`EventStore.PubSubBroker.startMessageListener: subscriptionClient.acknowledge(${messagesToAckIds.length}) is too slow: ${ackTimeDiff}ms`);
                        }
                        if (EVENT_STORE_STATS_PULL_ACK_LEN > 0) {
                            this.ackStats.push({
                            ...
                            this.ackStats = this.ackStats.slice(-1 * EVENT_STORE_STATS_PULL_ACK_LEN)
                        }
