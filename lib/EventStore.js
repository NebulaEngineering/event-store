...
const { filter, map, concatMap, switchMap, tap } = require('rxjs/operators');
...
class EventStore {
    ...
    emitEvent$(event) {
        ...
        if(event.et === "DistributionCenterAssigned"){
            initTime = Date.now();
            console.log(`(${event.aid}) DistributionCenterAssigned ==> (init): `, Date.now());            
        }        
        return this.storeDB.pushEvent$(event)
            .pipe(
                tap(() => {                    
                    if(event.et === "DistributionCenterAssigned"){
                        console.log(`(${event.aid}) DistributionCenterAssigned ==> (After pushEvent$): ${ Date.now() - initTime} ms`);            
                    } 
                }),
                concatMap((storeResult) =>
                    this.broker.publish$(storeResult.event)
                        .pipe(
                            tap(() => {
                                if(event.et === "DistributionCenterAssigned"){
                                    console.log(`(${event.aid}) DistributionCenterAssigned ==> (After publish$): ${Date.now() - initTime} ms`)            
                                }                                
                            }),
                            map(messageId => ({ storeResult, brokerResult: { messageId } }) )
                        )
                )
            );
    }
