...
const { of, merge, iif, forkJoin } = require('rxjs');
const { map, concatMap, switchMap, mergeMap } = require('rxjs/operators');
...
class EventStore {
    ...
    emitEvent$(event, { autoAcknowledgeKey } = {}) {
        ...
        event.av = event.timestamp;
        return forkJoin([
            this.storeDB.pushEvent$(event),
            this.broker.publish$(event)
        ]).pipe(
            map(([storeResult,brokerResult])=> ({ storeResult, brokerResult}))
        );
    }
