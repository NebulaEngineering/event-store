...
const { filter, map, concatMap, switchMap, mergeMap } = require('rxjs/operators');
...
class EventStore {
    ...
    emitEvent$(event, { autoAcknowledgeKey } = {}) {
        return this.storeDB.pushEvent$(event)
            .pipe(
                concatMap((storeResult) => this.broker.publish$(storeResult.event).pipe(
                    map(messageId => {
                ...
                mergeMap(x => Rx.iif(
                    () => autoAcknowledgeKey,
                    this.acknowledgeEvent$(event, autoAcknowledgeKey),
                    Rx.of(x)))
            );
    }
     ...
     * @param {string} allAggregateTypes 
     */
    getEventListener$(aggregateType, key, ignoreSelfEvents = true, allAggregateTypes) {
        return this.storeDB.findLatestAcknowledgedTimestamp$(aggregateType, key).pipe(
            switchMap(latestTimeStamp => {
                ...
                return this.broker.getEventListener$(aggregateType, ignoreSelfEvents, allAggregateTypes)
