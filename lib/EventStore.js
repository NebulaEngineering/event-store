...
class EventStore {
    ...
    getEventListener$(aggregateType, ignoreSelfEvents = true) {
        return this.storeDB.findLatestAcknowledgedTimestamp$(aggregateType, key)
            .switchMap(latestTimeStamp => {
                //this will ignore all queued messages that were already processed
                this.broker.getEventListener$(aggregateType, ignoreSelfEvents)
                    .filter(evt => evt.timestamp > latestTimeStamp)
            });
    }
    ...
    ensureAcknowledgeRegistry$(aggregateType) {
        return this.storeDB.ensureAcknowledgeRegistry$(aggregateType);
    }
}
