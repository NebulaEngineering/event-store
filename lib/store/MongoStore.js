    ...
    pushEvent$(event) {
        ...
        return this.incrementAggregateVersionAndGet$(event.at, event.aid, event.timestamp)
            .mergeMap(([aggregate, versionTimeStr]) => {
                ...
                return Rx.Observable.fromPromise(collection.insertOne(event))
    ...
    incrementAggregateVersionAndGet$(type, id, versionTime) {
        ...
        return this.getAggreate$(type, id, true, versionTime)
            .switchMap(findResult => {
                const index = findResult.index && findResult.index[versionTimeStr] ? findResult.index[versionTimeStr] : { initVersion: findResult.version ? findResult.version : 1, initTime: findResult.versionTime };
                ...
                Rx.Observable.from(Object.entries(aggregate.index))
                    ...
                    .map(([time, index]) => {
                        ...
                        return Rx.Observable.bindNodeCallback(collection.find.bind(collection))({
                            ...
                            .mergeMap(cursor => Rx.Observable.range(lowLimit, realLimit).mapTo(cursor))
                            .mergeMap(cursor => Rx.Observable.fromPromise(this.extractNextFromMongoCursor(cursor)))
                    })
                    .concatAll()
                    .filter(data => data)
