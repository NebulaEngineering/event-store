...
class MongoStore {
  ...
  incrementAggregateVersionAndGet$(type, id, versionTime) {
    ...
    if(type == "DistributionCenterAssigned"){
      console.log("incrementAggregateVersionAndGet$ (init) ==> ", Date.now(), `(${initTime})`);
    }
    if (!versionTime) {
    ...
    return this.getAggreate$(type, id, true, versionTime).pipe(
      tap(() => {
        if(type == "DistributionCenterAssigned"){
          console.log("After getAggreate$ => ", Date.now() - initTime  )
        }
      }),
      ...
      tap(() => {
        if(type == "DistributionCenterAssigned"){
          console.log("After findAndUpdate$ => ", Date.now() - initTime  )
        }
      }),
      pluck('value'),
