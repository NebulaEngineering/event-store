...
class MongoStore {
  ...
  pushEvent$(event) {
    ...
    if (!Array.isArray(event)) {
      ...
      const versionDate = new Date(event.timestamp);
      ...
      return defer(() => collection.insertOne(event, { writeConcern: { w: 0, wtimeout: 2000, j: false } })).pipe(
        mapTo(event)
      );
    }

    return from(event).pipe(
      map(evt => ({ evt, at: evt.at, versionTimeStr: new Date(evt.timestamp).getFullYear() + ('0' + (new Date(evt.timestamp).getMonth() + 1)).slice(-2) })),
      groupBy(({ at, versionTimeStr }) => `${at}=>${versionTimeStr}`),
      mergeMap(group => group.pipe(
        ...
        toArray(),
        mergeMap(evts => {
          const [at, versionTimeStr] = group.key.split('=>');
          return defer(() =>
            this.mongoClient
              .db(`${this.eventStoreDbName}_${versionTimeStr}`)
              .collection(at)
              .insertMany(evts, { writeConcern: { w: 0, wtimeout: 2000, j: false } }));
        })
      )),
      last(),
      mapTo(event)
