...
class MongoStore {
  ...
  pushEvent$(event) {
    ...
    let afterIncrementAggregateVersionAndGet = null;
    let afterPushEvent = null;
    if(event.et === "DistributionCenterAssigned"){        
        console.log(`(${event.aid}) pushEvent$ (init): `, Date.now());            
    } 
    if (!event.timestamp) {
    ...
    return this.incrementAggregateVersionAndGet$(event.at, event.aid, event.timestamp )
      .pipe(
        tap(() => {
          if(event.et === "DistributionCenterAssigned"){
            afterIncrementAggregateVersionAndGet = Date.now() - initTime
            console.log(`(${event.aid}) After incrementAggregateVersionAndGet$ ==> (${ afterIncrementAggregateVersionAndGet })}`);            
          } 
        }),
          ...
          return event.ephemeral
            ...
            : Rx.defer(() => collection.insertOne(event, { writeConcern: { w: 'majority', wtimeout: 500, j: true } }))
              .pipe(
                tap(() => {
                  if(event.et === "DistributionCenterAssigned"){
                    afterPushEvent = Date.now() - initTime;
                    console.log(`(${event.aid}) After InserEvent$ ==> (${ afterPushEvent })}`);            
                  } 
                }),
