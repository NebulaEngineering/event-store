...
class MongoStore {
   ...
   */
  start$() {
    return Rx.bindNodeCallback(MongoClient.connect)(this.url, { poolSize: 200 }).pipe(
      map(client => {
        this.mongoClient = client;
        this.aggregatesDb = this.mongoClient.db(this.aggregatesDbName);
        this.aggregateCollection = this.aggregatesDb.collection('Aggregates');
  ...
  pushEvent$(event) {
    ...
    if(event.et === "DistributionCenterAssigned"){        
        console.log(`(${event.aid}) pushEvent$ (init): `, initTime);            
    } 
    if (!event.timestamp) {
    ...
    return this.incrementAggregateVersionAndGet$(event.at, event.aid, event.timestamp )
      .pipe(
        // tap(() => {
        //   if(event.et === "DistributionCenterAssigned"){
        //     console.log(`(${event.aid}) After incrementAggregateVersionAndGet$ ==> (${ (Date.now() - initTime) })}`);            
        //   } 
        // }),
        mergeMap(([aggregate, versionTimeStr]) => {
          ...
          return event.ephemeral
            ...
            : Rx.defer(() => collection.insertOne(event, { writeConcern: { w: 'majority', wtimeout: 500, j: true } }))
              .pipe(
                tap(() => {
                  if(event.et === "DistributionCenterAssigned"){
                    console.log(`(${event.aid}) After InserEvent$ ==> (${ (Date.now() - initTime) })}`);            
                  } 
                }),
  ...
  incrementAggregateVersionAndGet$(type, id, versionTime) {
    ...
    if(type == "Orderbox"){
      console.log(`(${id}) incrementAggregateVersionAndGet$ (init) ==> ${ Date.now()} `);
    }
    if (!versionTime) {
    ...
    return this.getAggreate$(type, id, true, versionTime).pipe(
      tap(() => {
        if(type == "Orderbox"){
          console.log(`(${id}) After getAggreate$ => (${Date.now() - initTime})`)
        }
      }),
      switchMap(findResult => {
        ...
        // return Rx.bindNodeCallback( this.aggregateCollection.findOneAndUpdate.bind(this.aggregateCollection) )({ type, id }, update, {
        //   upsert: true,
        //   returnOriginal: false
        // });
        return Rx.defer(() => this.aggregateCollection.findOneAndUpdate(
          { type, id }, update,
          {
            upsert: true,
            returnOriginal: false
      ...
      tap(() => {
        if(type == "Orderbox"){
          console.log(`${id} After findAndUpdate$ => ${Date.now() - initTime}`,   )
        }
      }),
      pluck('value'),
