...
class MongoStore {
    ...
    retrieveUnacknowledgedEvents$(aggregateType, key) {
        ...
        */
        ...
        return this.findLatestAcknowledgedTimestamp$(aggregateType, key) // get latest ack timestamp
    ...
    findLatestAcknowledgedTimestamp$(aggregateType, key) {
        ...
        //lets build all const queries so the RxJS stream is more legible
        return Rx.Observable.defer(() => this.aggregatesDb.collection('Acknowledges').findOne(findSearchQuery, findSearchProjection))
            .map(findResult => {
