  ...
  findLatestAcknowledgedTimestamp$(aggregateType, key) {
    ...
    return Rx.defer(() =>
      this.aggregatesDb
        ...
        .findOne(findSearchQuery, findSearchProjection)
    ).pipe(
      map(findResult => {
    ...
    return Rx.defer(() =>
      ...
      mergeMap(updateResult => {
        return updateResult.upsertedCount > 0 // checks if the insert was made
          ? Rx.of(
              `EventStore has created an empty registry for ${aggregateType}`
            )
          : Rx.of(
              `EventStore has verified the existence of a registry for ${aggregateType}`
            );
      })
    );
  }
    ...
    return Rx.defer(() =>
      ...
      mergeMap(updateResult => {
        return updateResult.nModified > 0 // checks if the update was made
          ? Rx.of(event) // if updated then returns the event
          : updateResult.n > 0 // checks if the document was found even if it was not updated
            ? Rx.throw(
                new Error(
                  'Error while persisting event acknowledge, the document was found but no modification was done'
                )
              ) // if the document was found but not modified then is an error
            : Rx.defer(() =>
                collection.findOne(findSearchQuery, findSearchProjection)
              ) // attemps to retrieve the document to check if it does exists or is a version ahead
                .pipe(
                  mergeMap(findResult => {
                    return !findResult // chechks if the document was not found
                      ? Rx.defer(() =>
                          collection.insertOne(
                            insertEntireDocumentQuery,
                            writeConcern
                          )
                        ) // the document does not exists, so it can be created as new
                          .pipe(mapTo(event)) //returns the event
                      : findResult.ack[key] // the document does exists, lets chek if key exists
                        ? Rx.of(event) // the key exists, so no need to create it.  just return the event
                        : Rx.defer(() =>
                          ...
                          ) // the key does not exists, so lets update the documento so the key is created
                            .pipe(mapTo(event)); //returns the event
                  })
                );
      })
